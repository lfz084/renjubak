...
checkerBoard.prototype.getSVG = function() {
  ...
  for (let i = 0; i < this.SLTX; i++) {
    for (let j = 0; j <= this.SLTX * (this.SLTY - 1); j += this.SLTX * (this.SLTY - 1)) {
      ...
      svgText += ` <text x="${x1}" y="${y1}" font-weight="bold" font-family="SimHei,STHeiti,Source Han Sans CN"  font-size="${this.gW*0.5*size}" text-anchor="middle" dominant-baseline="central">${this.alpha.charAt(i)}</text>`;
  ...
  for (let i = 0; i < this.SLTY; i++) {
    for (let j = 0; j <= this.SLTX - 1; j += this.SLTX - 1) {
      ...
      svgText += ` <text x="${x1}" y="${y1}" font-weight="bold" font-family="SimHei,STHeiti,Source Han Sans CN" font-size="${this.gW*0.5*size}" text-anchor="middle" dominant-baseline="central">${String(this.SLTY-i)}</text>`;
  ...
  for (let i = 0; i < this.SLTY * this.SLTX; i++) {
    ...
    if (this.P[i].type == tNum || this.P[i].type == tWhite || this.P[i].type == tBlack) {
      ...
      /*
       if (this.P[i].type==tNum) {
           ...
           svgText += ` <text x="${this.P[i].x*size}" y="${this.P[i].y*size}" stroke="${color}" fill="${color}" font-weight="bold" font-family="SimHei,STHeiti,Source Han Sans CN" font-size="${this.gW*0.5*size}" text-anchor="middle" dominant-baseline="central">${this.P[i].text}</text>`;
       }
      */
    }
      ...
      //svgText += ` <text x="${this.P[i].x*size}" y="${this.P[i].y*size}" stroke="${this.P[i].color}" fill="${this.P[i].color}" font-weight="bolder" font-family="SimHei,STHeiti,Source Han Sans CN" font-size="${this.gW*0.5*size}" text-anchor="middle" dominant-baseline="central">${this.P[i].text}</text>`;
    }
    ...
    svgText += ` <text x="${x1*size}" y="${y1*size}" stroke="${color}" fill="${color}" font-weight="bolder" font-family="SimHei,STHeiti,Source Han Sans CN" font-size="${fontsize*size}" text-anchor="middle" dominant-baseline="central">${txt}</text>`;
  }
...
checkerBoard.prototype.printCoordinate = function(t, r, d, l) {
  ...
  ctx.fillStyle = "#000";
  ctx.font = parseInt(this.gW * 0.5) + "px SimHei,STHeiti,Source Han Sans CN";
...
checkerBoard.prototype.printPoint = function(idx, text, color, type, showNum) {
  ...
  if (type == tNum || type == tWhite || type == tBlack) {
    ...
    ctx.font = "bolder " + parseInt(w * 1.08) + "px  SimHei,STHeiti,Source Han Sans CN";
  }
  else { //  打印标签
    ctx.beginPath();
    ctx.fillStyle = this.backgroundColor;
    ...
    ctx.fill();
    ctx.fillStyle = color;
    ctx.font = "bolder " + parseInt(w * 1.1) + "px  SimHei,STHeiti,Source Han Sans CN";
  }
  ...
  if (text.length == 1) { // 两位数数数字不需要放大字体
    ...
    if (code < "0".charCodeAt() || code > "9".charCodeAt()) {
      if (text == "▲" || text == "■" || text == "☆" || text == "◎" || text == "✖") {
        ...
        ctx.font = "bolder " + parseInt(w * 1.5) + "px  SimHei,STHeiti,Source Han Sans CN";
      }
    }
  }
