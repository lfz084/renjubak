...
let engine = (() => {
    ...
    let createWork = () => {
        let wk = new Worker("./script/engine-20200808.js");
        wk.onmessage = (e) => {
        ...
        postMsg: (cmd, param) => {
            ...
            let cmdList = {
                ...
                "isTwoVCF": () => {
                    ...
                    let color = param[1];
                    let newarr = param[2];
                    work = new Worker("./script/engine-20200808.js");
                    work.onmessage = (e) => {
                        labelTime.setPrePostTimer = new Date().getTime();
                        newarr = e.data.parameter[0];
                        continuefun();
                    };
                    work.postMessage({ "cmd": "selectPoint", parameter: [arr, color, newarr, null, null, true, { level: 2 }] });
                    ...
                    let continuefun = () => {
                        ...
                        for (let i = 0; i < maxThread; i++) {
                            if (sPoint.length) {
                                works[i] = new Worker("./script/engine-20200808.js");
                                workCount++;
                                works[i].onmessage = (e) => {
                                    labelTime.setPrePostTimer = new Date().getTime();
                ...
                "getBlockVCFb": () => {
                    ...
                    work.onmessage = (e) => {
                        ...
                        else if (command == "getBlockVCFEnd") {
                            closeMsg();
                            work.terminate();
                            sPoint = p[0];
                            if (sPoint.length) {
            ...
            cmdList.isLevelThreePoint = cmdList.isSimpleWin = cmdList.isTwoVCF;
            save(cBd);
            setCmd();
