...
function findFourWin(arr, color, newarr) {
    let fPoint = findfour(arr, color, newarr) || [];
    let wPoint = [];
    for (let i=fPoint.length-1; i>=0; i--) {
        let x = fPoint[i] % 15;
        let y = parseInt(fPoint[i]/15);
        if (isFFWin(x, y, color, arr)) {
            wPoint.push(sPoint[i]);
            break;
        }
    }
    return wPoint;
...
function findThreeWin(arr, color, newarr) {
    ...
    for (let i=tPoint.length-1; i>=0; i--) {
        //alert(tPoint[i] + "   " + i);
        let x = tPoint[i] % 15;
        let y = parseInt(tPoint[i] / 15);
        if (isTTWin(x, y, color, arr)) {
            wPoint.push(tPoint[i]);
            break;
        }
    }
    return wPoint;
...
function isSimpleWin(idx, arr, color, num, level) {
    if (num == 4) {
        if (level == 3) {  //大道五目
            let timeout = 30000;
            let depth = 1;
            ...
            isSimpleWinB(idx, arr, color, num);
        }
    }
    post("end");
}
...
// 四手五连
function isSimpleWinB(idx, arr, color, num) {
    if (num == 4) {
        let timeout = 30000;
        let depth = 1;
        let x = idx % 15;
        ...
        //console.log(1);
        arr[y][x] = color;
        let narr = selectPoint(arr, color==1?2:1, getArr([]), timeout, null, true, null, true);
    ...
    function findWin(arr, color, timeout, depth, gDepth, maxNum) {
        let narr = selectPoint(arr, color, getArr([]), timeout);
        for (let y = 0; y < 15; y++) {
            for (let x = 0; x < 15; x++) {
                if (narr[y][x] == 0 && arr[y][x] == 0) {
                    //console.log(narr);
...
// 确定选点范围
function selectPoint(arr, color, newarr, timeout, depth, backstage, level, allColor, num) {
    num = typeof(num)=="number"?parseInt(num):9999;
    timeout = timeout || 30000;
    depth = depth || 1000;
    backstage = backstage == null ? true : backstage;
